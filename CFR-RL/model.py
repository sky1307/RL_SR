...
class Network():
    ...
    def create_actor_critic_model(self, config):
        ...
        inputs = tf.keras.Input(shape=(self.input_dims[0], self.input_dims[1]))
        # Actor
        x_inputs = tf.keras.layers.Flatten()(inputs)
        Dense0_1 = tf.keras.layers.Dense(config.Dense_0)
        x_1 = Dense0_1(x_inputs)
        ...
        # Critic
        Dense0_2 = tf.keras.layers.Dense(config.Dense_0)
        x_2 = Dense0_2(x_inputs)
    ...
    def value_loss_fn(self, rewards, values):
        # print("reward, values ",rewards, values)
